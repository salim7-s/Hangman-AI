export default function ResultModal({ status, word, streak, bestStreak, onPlayAgain, mode }) {
  if (!status || status === 'ongoing') return null

  // In player-vs-ai (reverse), the player wins if the AI fails to guess (status === 'lost')
  ...
  return (
    <div className="fixed inset-0 z-50 overflow-y-auto bg-[#2c2825]/80 px-3 py-3 backdrop-blur-sm sm:flex sm:items-center sm:justify-center sm:px-4">
      <div className="glass-panel pop mx-auto w-full max-w-[calc(100vw-1.5rem)] sm:max-w-xl md:max-w-2xl overflow-x-hidden p-4 max-sm:min-h-fit max-sm:shadow-[4px_4px_0px_rgba(44,40,37,0.22)] sm:max-h-[calc(100dvh-2rem)] sm:p-12 sm:rotate-[2deg]">
        {/* Fake staple */}
